...
class PurchaseServiceTest {
    ...
    fun `purchase establishment deducts coins updates ownership and supply`() {
        ...
        whenever(gameStateGuard.ensureGameIsRunning(gameId))
            .thenReturn(activeGame(gameId))

        whenever(playerDao.getPlayers(gameId))
            .thenReturn(listOf(player(gameId, coins = 5)))

        whenever(cardDao.findByCardType(CardType.BAKERY))
            .thenReturn(
                CardModel(
                    1,
                    CardType.BAKERY,
        ...
        whenever(gameMarketplaceDao.findByGameIdAndType(gameId, CardType.BAKERY))
            .thenReturn(
        ...
        whenever(playerCardDao.findByPlayerId(10))
    ...
    fun `purchase landmark deducts coins and marks landmark built`() {
        ...
        whenever(playerDao.getPlayers(gameId))
            .thenReturn(listOf(player(gameId, coins = 6)))

        whenever(landmarkDao.findByLandmarkType(LandmarkType.TRAIN_STATION))
            .thenReturn(
                LandmarkModel(
                    1,
        ...
        whenever(
            playerLandmarkDao.findByPlayerIdAndType(
                10,
                LandmarkType.TRAIN_STATION
            )
        ).thenReturn(
    ...
    fun `purchase rejects establishment when coins are insufficient`() {
        ...
        whenever(playerDao.getPlayers(gameId))
            .thenReturn(listOf(player(gameId, coins = 1)))

        whenever(cardDao.findByCardType(CardType.STADIUM))
            .thenReturn(
                CardModel(
                    1,
                    CardType.STADIUM,
    ...
    fun `purchase rejects establishment when supply is unavailable`() {
        ...
        whenever(playerDao.getPlayers(gameId))
            .thenReturn(listOf(player(gameId)))
